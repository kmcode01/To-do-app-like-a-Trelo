...
async function bootstrap() {
  ...
  const { data: members, error: membersError } = await supabase
    ...
    .select("user_id, role, created_at, user_name")
      ...
      <main class="page-content">
        ...
        <section class="detail-card members-card" aria-label="Project users">
          <div class="section-head">
            <div>
              <h2>Project Users</h2>
              <p class="section-subtitle">Assign or remove users for this project.</p>
            </div>
  ...
  async function reloadMembers() {
    const { data: latestMembers, error: refreshError } = await supabase
      .from("project_members")
      .select("user_id, role, created_at, user_name")
      .eq("project_id", projectId)
      .order("created_at", { ascending: true });
  ...
  if (userList) {
    userList.addEventListener("click", async (event) => {
      ...
      if (insertError) {
        ...
        if (addMessage) {
          addMessage.textContent = insertError.message;
  ...
  if (confirmRemoveButton && removeDialog) {
    confirmRemoveButton.addEventListener("click", async () => {
      ...
      if (deleteError) {
        if (removeMessage) {
          removeMessage.textContent = deleteError.message;
        }
        return;
      }
